...
**Correction vidéo, de Lovable au durcissement final.** Lovable a d'abord fiabilisé la sortie : écoute de plusieurs variantes d'événement de fin Gumlet et fallback sur la progression à moins de 0,4 s de la durée. Le teaser conserve l'iframe Gumlet préchargée sans contrôles. Une dernière investigation a identifié deux courses qui échappaient aux anciens tests : l'activation visuelle attendait la fin de l'authentification et un callback `ready` capturant encore `active=false` pouvait repasser le player en pause ; inversement, « Passer » ne faisait qu'envoyer des commandes Player.js asynchrones et ne coupait rien si l'objet n'était pas prêt. Le clic initial passe maintenant immédiatement en phase teaser, demande l'autoplay sonore et conserve cette intention jusqu'à `ready`. Le skip effectue d'abord un hard-stop : mute/pause/seek, détachement HLS et suppression de source, ou destruction synchrone de l'iframe via `about:blank`, avant seulement de changer d'écran. Les interludes restent en autoplay sans contrôles, avec piste audio et volume forcé.

**Compound loop anti-régression.** Les incidents ne restent plus de simples notes : ils deviennent des invariants exécutables dans `docs/core_experience_regression_contract.md`. Une suite critique nommée couvre séparément Deepgram, Gamilab, la finalisation, le budget Max, l'audio et la vidéo ; la CI GitHub exécute ensuite les tests unitaires, le build et six histoires Playwright jusqu'à 35 tours. Deux scénarios média instrumentent maintenant le protocole Player.js et `HTMLMediaElement` : ils exigent `playing=true`, `muted=false` après le démarrage, puis `playing=false` et un arrêt observé après « Passer », séparément pour l'iframe teaser et le HLS natif. Cette boucle a aussi détecté un faux `MediaRecorder` incomplet et un ID Gumlet fictif partiellement accepté par le parseur.
...
**Validation.** 102 tests unitaires, 29 contrats critiques, build Vite, lint ciblé et six scénarios Playwright sont verts. Les parcours prouvent l'autoplay sonore et l'arrêt sur skip des deux modes vidéo, trois tours successifs, une réponse audio longue non coupée, un retry ElevenLabs et 35 tours avec panne RAG, panne LLM récupérable et cinématique HLS. Les tests de provider prouvent aussi que le réglage Gamilab ne reçoit pas le modèle Deepgram.
...
**Files.** `src/services/deepgramSTT.ts`, `src/services/stt/`, `src/services/prd4Orchestrator.ts`, `src/config/experienceRuntime.ts`, `supabase/functions/proxy-stt/index.ts`, `src/components/GumletVideoPlayer.tsx`, `src/pages/IndexPRD4.tsx`, `tests/e2e/prd4-happy-path.spec.ts`, `.github/workflows/core-experience-quality.yml`, `docs/core_experience_regression_contract.md`, `bun.lock`.
