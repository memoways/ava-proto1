...
**Correction vidéo, de Lovable au durcissement final.** Lovable a d'abord fiabilisé la sortie : écoute de plusieurs variantes d'événement de fin Gumlet et fallback sur la progression à moins de 0,4 s de la durée. Le teaser conserve finalement l'iframe Gumlet préchargée qui fonctionnait déjà, sans contrôles et avec son autoplay interne désactivé parce que Gumlet le force en mode muet : le clic initial « Commencer » lance automatiquement la vidéo par `play`, volume à 100 %, `unmute`, puis `play` dans la pile du geste utilisateur et déverrouille l'audio pour la session, sans écran d'activation ni bouton Play. Les interludes disposent d'un player HLS natif séparé et persistant, toujours en autoplay sans contrôles, avec piste audio, volume forcé et retries automatiques au chargement. « Passer » impose désormais `pause` et un retour à zéro avant la transition, afin qu'aucun son ne continue en arrière-plan. Les messages de fin provenant d'autres fenêtres sont ignorés.

**Compound loop anti-régression.** Les incidents ne restent plus de simples notes : ils deviennent des invariants exécutables dans `docs/core_experience_regression_contract.md`. Une suite critique nommée couvre séparément Deepgram, Gamilab, la finalisation, le budget Max, l'audio et la vidéo ; la CI GitHub exécute ensuite les tests unitaires, le build et quatre histoires Playwright jusqu'à 35 tours. Cette première exécution a déjà détecté un faux `MediaRecorder` de test devenu incomplet et un ID Gumlet fictif partiellement accepté par le parseur ; le double a été aligné sur le navigateur et le parseur exige maintenant les 24 caractères hexadécimaux complets.
