...
**Correction vidéo, de Lovable au durcissement final.** Lovable a d'abord fiabilisé la sortie : écoute de plusieurs variantes d'événement de fin Gumlet et fallback sur la progression à moins de 0,4 s de la durée. Le teaser conserve finalement l'iframe Gumlet préchargée qui fonctionnait déjà : le premier clic lui envoie directement `play`, `unmute` et volume à 100 % dans la pile du geste utilisateur, sans écran d'activation supplémentaire. Les interludes disposent d'un player HLS natif séparé et persistant, avec piste audio, volume forcé et retries automatiques au chargement. Les messages de fin provenant d'autres fenêtres sont ignorés.
