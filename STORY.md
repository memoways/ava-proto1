...
**Correction vidéo, de Lovable au durcissement final.** Lovable a d'abord fiabilisé la sortie : écoute de plusieurs variantes d'événement de fin Gumlet et fallback sur la progression à moins de 0,4 s de la durée. Le teaser conserve finalement l'iframe Gumlet préchargée qui fonctionnait déjà, configurée en autoplay sans contrôles : le premier clic lui envoie directement `play`, `unmute` et volume à 100 % dans la pile du geste utilisateur, sans écran d'activation ni bouton Play. Les interludes disposent d'un player HLS natif séparé et persistant, toujours en autoplay sans contrôles, avec piste audio, volume forcé et retries automatiques au chargement. « Passer » impose désormais `pause` et un retour à zéro avant la transition, afin qu'aucun son ne continue en arrière-plan. Les messages de fin provenant d'autres fenêtres sont ignorés.
