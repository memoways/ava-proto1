...
sampling / RAG, 3 passages, classement Δ vs live. Les modèles comparés sont
le catalogue de LLM Config (pas une liste parallèle).
...
**Correction STT multi-provider.** `STTSession.flush()` devient une barrière asynchrone commune et bornée. Deepgram récupère la queue audio, envoie `Finalize`, attend la révision puis réunit préfixe final et queue interim sans duplication. AssemblyAI envoie `ForceEndpoint` et conserve son transcript cumulatif. Gamilab reconstruit l'intégralité à partir de `text_history + text_current`, réémet le texte complet à chaque correction et ignore les placeholders d'activité `…` qui avaient fini par remplacer la parole visible et enregistrée. Whisper et Gradium attendent le dernier blob avant l'appel batch. Le mode PTT manuel interdit aussi une soumission anticipée sur simple silence. Pendant cette courte fenêtre, l'UI affiche *« Finalisation de tout ce qui a été dit… »* et bloque le bouton ; la conversation, la capture du rôle et la capture de posture suivent toutes cette règle. Le texte utilisateur reste ensuite affiché pendant que Max réfléchit et répond. L'audit a en outre montré que Whisper et AssemblyAI, bien que déclarés implémentés, n'étaient pas instanciés par la factory : ce routage est rétabli.

**Incident Deepgram apparent, cause LLM réelle.** Une capture semblait montrer une nouvelle panne Deepgram, mais les logs prouvaient `WebSocket connected` puis une fermeture normale. Deux instruments brouillaient le diagnostic : l'overlay conservait `nova-2` en dur alors que le proxy utilisait `nova-3`, et sa durée STT incluait tout le temps de parole. Le texte « la ligne accroche » venait en réalité de Max : le budget global de 5 s, après 2 s de RAG, réduisait son timeout à environ 2,8 s. Le modèle Deepgram par défaut et la langue sont désormais partagés entre proxy et navigateur, la connexion journalise sa configuration effective, et Max retrouve une fenêtre de 8 s dans un budget de 11 s. Un test impose qu'une génération de 3,5 s après un RAG bloqué reste une vraie réponse.

**Correction vidéo, de Lovable au durcissement final.** Lovable a d'abord fiabilisé la sortie de l'embed, mais l'investigation a établi une incompatibilité structurelle : Gumlet force muet l'autoplay dans son iframe cross-origin et peut afficher sa propre gate sonore, tandis que l'activation du clic parent ne traverse pas fiablement cette frontière. L'iframe et Player.js sont donc retirés du parcours. Un seul élément `<video>` natif est préchargé avant l'accueil et conservé pour le teaser comme pour tous les interludes ; les URLs `watch/embed` deviennent des manifestes HLS directs. Une première mise en production a révélé un second piège : Chromium/Brave annonçait `canPlayType(m3u8)="maybe"`, recevait les manifestes, puis échouait avec `MediaError.code=4`. L'ordre officiel hls.js est maintenant respecté : `Hls.isSupported()` en premier, HLS natif uniquement en fallback, et « Commencer » attend le manifeste parsé. Le clic applique ensuite `muted=false`, volume maximal et `play()` avant l'authentification. Les vidéos suivantes ne font que changer la source de ce même élément déjà autorisé. « Passer » mute et pause d'abord, remet à zéro, détruit HLS et retire la source avant la transition ; les anciennes promesses de lecture sont invalidées par génération.

**Compound loop anti-régression.** Les incidents ne restent plus de simples notes : ils deviennent des invariants exécutables dans `docs/core_experience_regression_contract.md`. Une suite critique nommée couvre séparément Deepgram, Gamilab, la finalisation, le budget Max, l'audio et la vidéo ; la CI GitHub exécute ensuite les tests unitaires, le build et six histoires Playwright jusqu'à 35 tours. Les scénarios média n'acceptent plus un faux Player.js : ils interdisent toute iframe, vérifient que le même nœud `<video>` sert au teaser et à l'interlude, exigent `autoplay`, `muted=false`, volume maximal et lecture observée, puis contrôlent que « Passer » provoque une pause avant la transition.
...
**Validation.** 101 tests unitaires, 28 contrats critiques, build Vite, lint ciblé, six scénarios Chromium complets et quatre variantes vidéo Firefox/WebKit sont verts. Les parcours prouvent l'absence d'iframe, la persistance du même lecteur, la source `blob:` hls.js sous Chromium/Firefox, l'autoplay sonore et l'arrêt sur skip, trois tours successifs, une réponse audio longue non coupée, un retry ElevenLabs et 35 tours avec panne RAG, panne LLM récupérable et cinématique HLS. Un smoke test sans mock sur le vrai média Gumlet mesure aussi `readyState=4`, 77 s de durée, une progression effective, `muted=false`, volume maximal et aucune erreur média. Les tests de provider prouvent enfin que le réglage Gamilab ne reçoit pas le modèle Deepgram.
...
**Files.** `src/services/deepgramSTT.ts`, `src/services/stt/`, `src/services/prd4Orchestrator.ts`, `src/services/videoPlayback.ts`, `src/config/experienceRuntime.ts`, `supabase/functions/proxy-stt/index.ts`, `src/components/GumletVideoPlayer.tsx`, `src/pages/IndexPRD4.tsx`, `tests/e2e/prd4-happy-path.spec.ts`, `playwright.config.ts`, `.github/workflows/core-experience-quality.yml`, `docs/core_experience_regression_contract.md`, `package.json`, `bun.lock`.
