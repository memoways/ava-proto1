    ...
    <div className="flex min-h-screen flex-col items-center justify-center bg-background px-6 py-10">
      <div className="mx-auto w-full max-w-3xl space-y-8 text-center">
        ...
        <div className="grid grid-cols-2 gap-4 sm:gap-6 md:grid-cols-4">
          {CHARACTERS.map((c) => {
            const cardBase =
              "group relative flex flex-col items-center gap-3 rounded-lg border bg-card/60 p-4 transition";
            if (c.active) {
              return (
                <button
                  key={c.id}
                  onClick={onSelectMax}
                  className={`${cardBase} border-primary/50 hover:border-primary hover:bg-card`}
            ...
            return (
              <button
                ...
                className={`${cardBase} border-border opacity-50 grayscale hover:opacity-70`}
              ...
              >
                ...
                <div>
                  ...
                  <p className="text-xs text-muted-foreground/80">Bientôt</p>
                </div>
              </button>
            );
          })}
